// LEGACY DUMP OF THE MAIN PROGRAM FILE FOR REFERENCE PURPOSES ONLY.
#if false
...
class Program
{
    ...
    static void Main()
    {
        ...
        // ============================
        Console.WriteLine("=== Galenvale Text RPG ===");
        ...
        if (hasSave && save != null)
        {
            ...
            Console.Write("> ");
            string choice = GetRawInput(allowEmpty: false).Trim();
            continuing = (choice != "2");
            ...
            if (choice == "2")
            {
                ...
                save = null;
                hasSave = false;
                Console.WriteLine("Save cleared. Starting a NEW game...");
                Console.WriteLine();
                continuing = false;
        ...
        else
        {
            ...
            Console.Write("> ");
            _ = GetRawInput(allowEmpty: false);
            Console.WriteLine();
        }
        ...
        if (continuing && save != null)
        {
            // --- LOAD RUNTIME STATE FROM SAVE ---
            playerNameUi = save.PlayerNameUi;
            playerClass = save.PlayerClass;

            // Base stats derived from class (stable even if you rebalance later)
            ...
            // Until you add OwnedGearKeys to SaveData, gear restore cannot be correct.
            // So keep gear empty for now and allow the player to re-buy/re-equip.
            enemyCritReduction = 0;

            // Per your design: load/respawn = full restore + clean slate
            health = maxHealth;
            executeCooldown = 0;
            shockCooldown = 0;
            healCooldown = 0;
        ...
        else
        {
            ...
            playerClass = SelectClass(playerNameUi);
        ...
        // ============================
        // FIRST TOWN VISIT (from save or new game)
        // ============================
        string townToVisit = (continuing && save != null) ? save.CurrentTownId : "HARTLYN";

        VisitTown(
            townName: townToVisit,
            playerClass: playerClass,
            ref gold,
            ...
            gearForThisTown: townToVisit == "GALENWOOD" ? galenwoodShop
                         : townToVisit == "ROYAL CASTLE" ? royalShop
                         : hartlynShop,
            hasPotionShop: townToVisit != "HARTLYN",
            hasHeal20Potion: townToVisit != "HARTLYN"
        );
    ...
    hasHeal20Potion: false
        ...
        RunStageWithHubReturn(
            ...
            encounterStageIndex: 1,
            enemyFactory: () => MakeBandits(1),
            rewardGold: 50,
            allowPotions: false,
            hasHeal20Potion: false,
            playerClass: playerClass,
    ...
    hasHeal20Potion: true
        ...
        RunStageWithHubReturn(
            ...
            enemyFactory: () =>
            {
                ...
                e.WeaknessChance = 100; // your combat logic should ignore weakness application if player is PRIEST
                return new List<Enemy> { e };
            },
            rewardGold: 1000,
            allowPotions: true,
        ...
        // 4) GREAT KING (final) — use RunStageWithHubReturn so retries work cleanly
        RunStageWithHubReturn(
            "ROYAL CASTLE",
            "THE GREAT KING",
            ...
            rewardGold: 0,              // you can set a gold reward here but it's the final fight
            allowPotions: true,
            hasHeal20Potion: true,
            playerClass,
            playerNameUi,
        ...
        // Victory text AFTER you beat the Great King
        Console.WriteLine();
    ...
    // RETURNS: true = won, false = died (so caller can handle retry/save)
    // =========================================================
    static bool RunEncounter(
 ...
 ref int weaknessTurns
...

    {
        ...
        while (true)
        {
            ...
            // DIED -> return false (NO Environment.Exit)
            if (health <= 0)
            {
                Console.WriteLine();
            ...
            // If Shock (or a non-turn action) didn't use a turn, do NOT advance enemy/timers
            if (!usedTurn)
                continue;
...
ref int healCooldown
    ...
    {
        Console.WriteLine();
        Console.WriteLine("Choose a potion: HEAL10 | HEAL20 | RESET | BACK");
        Console.WriteLine($"YOU HAVE: HEAL10 x{potionHeal10Count} | HEAL20 x{potionHeal20Count} | RESET x{potionResetCooldownCount}");
        Console.Write("> ");

        string pick = GetNormalizedCommand();

        if (pick == "BACK") return false;

        if (pick == "HEAL10")
        {
            if (potionHeal10Count <= 0) { Narrate("You have none."); return false; }
            potionHeal10Count--;
            health = Math.Min(maxHealth, health + 10);
            Narrate($"You drink a HEAL10 potion. HP is now {health}/{maxHealth}.");
            return true;
        }

        if (pick == "HEAL20")
        {
            if (!hasHeal20Potion) { Narrate("That potion isn’t available here."); return false; }
            if (potionHeal20Count <= 0) { Narrate("You have none."); return false; }
            potionHeal20Count--;
            health = Math.Min(maxHealth, health + 20);
            Narrate($"You drink a HEAL20 potion. HP is now {health}/{maxHealth}.");
            return true;
        }

        if (pick == "RESET")
        {
            if (potionResetCooldownCount <= 0) { Narrate("You have none."); return false; }
            potionResetCooldownCount--;
            executeCooldown = 0;
            shockCooldown = 0;
            healCooldown = 0;
            Narrate("You drink a RESET potion. Your ability cooldown is restored!");
            return true;
        }
    ...
    // =========================================================
    // STAGE WRAPPER: save checkpoint -> attempt -> if die, return to hub + retry
    // Uses enemyFactory so enemies are always rebuilt fresh each attempt.
    // =========================================================
    static void RunStageWithHubReturn(
 string hubName,
 string stageTitle,
 int encounterStageIndex,
 Func<List<Enemy>> enemyFactory,   // ✅ IMPORTANT CHANGE
 int rewardGold,
 bool allowPotions,
 bool hasHeal20Potion,
 PlayerClass playerClass,
 ...
 bool hubHasHeal20Potion
    ...
    {
        // (Optional) Save a minimal checkpoint here if you want.
        // SaveSystem.Save(new SaveData { CurrentTown = hubName, CurrentStageTitle = stageTitle });
        ...
        while (true)
        {
            // ✅ Always rebuild enemies fresh for each attempt
            List<Enemy> enemies = enemyFactory();

            bool won = RunEncounter(
                stageTitle,
                enemies,
                rewardGold,
                allowPotions,
            ...
            // If your RunEncounter currently hard-exits on death, change it to RETURN false instead.
            // (I can give you that exact edit too, if you paste your current RunEncounter death block.)

            Console.WriteLine();
            Narrate($"You awaken back in {hubName}.");
            ...
            Narrate("You steady yourself and return to the fight...");
            // loop repeats, enemies rebuilt fresh
        }

        // After you WIN the stage: hub return or continue
        while (true)
        {
            Console.WriteLine();
    ...
    )
    {
        ...
        while (true)
        {
            ...
            Console.WriteLine($"POTIONS: HEAL10 x{potionHeal10Count} | HEAL20 x{potionHeal20Count} | RESET x{potionResetCooldownCount}");
    ...
    {
        const int heal10Cost = 10;
        const int heal20Cost = 25;
        const int resetCost = 40;
        ...
        while (true)
        {
            ...
            Console.WriteLine($"YOU HAVE: HEAL10 x{potionHeal10Count} | HEAL20 x{potionHeal20Count} | RESET x{potionResetCooldownCount}");
            Console.WriteLine();
            Console.WriteLine($"1) BUY HEALING POTION (10 HP)  - {heal10Cost} GOLD");
            if (hasHeal20Potion) Console.WriteLine($"2) BUY HEALING POTION (20 HP)  - {heal20Cost} GOLD");
            Console.WriteLine($"3) BUY RESET COOLDOWN POTION   - {resetCost} GOLD");
            Console.WriteLine("4) BACK");
            Console.Write("> ");

            string input = GetRawInput(allowEmpty: false).Trim();
            ...
            if (input == "1")
            {
                if (gold < heal10Cost) { Narrate("Not enough gold."); continue; }
                gold -= heal10Cost;
                potionHeal10Count++;
                Narrate("You buy a HEAL10 potion.");
            }
            else if (input == "2" && hasHeal20Potion)
            {
                if (gold < heal20Cost) { Narrate("Not enough gold."); continue; }
                gold -= heal20Cost;
                potionHeal20Count++;
                Narrate("You buy a HEAL20 potion.");
            }
            else if (input == "3")
            {
                if (gold < resetCost) { Narrate("Not enough gold."); continue; }
                gold -= resetCost;
                potionResetCooldownCount++;
                Narrate("You buy a RESET potion.");
            }
            else
            {
                Narrate("Invalid choice.");
